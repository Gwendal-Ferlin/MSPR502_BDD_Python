...
L’**inscription** (`POST /api/utilisateurs`) est **publique** (sans token). Les autres routes du tableau exigent un **JWT** valide.

**Mot de passe (RGPD, art. 32 — mesures appropriées ; recommandations CNIL)** : à l’inscription et lors d’un changement via `PATCH /api/utilisateurs/me`, le mot de passe doit avoir **au moins 12 caractères**, contenir une **majuscule**, une **minuscule**, un **chiffre** et un **caractère spécial** (ponctuation ASCII), ne pas reprendre des **formulations trop courantes** (liste côté serveur dans `api/services/password_policy.py`) ni la **partie locale de l’e-mail** (avant `@`). Réponse en cas de refus : **400** avec `detail.message` et `detail.exigences` (liste de messages). Les comptes du **seed** (`02_seed.sql`) gardent le mot de passe de démo `password` pour les tests ; ils ne passeraient pas cette politique s’ils étaient recréés via l’API.

| Méthode | Chemin                                       | Rôle               | Logué                                           | Description                                                                                                                                                                                                                                                                     |
| ------- | -------------------------------------------- | ------------------ | ----------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| POST    | `/api/utilisateurs`                          | Public             | Non                                             | Création de compte (rôle Client, Freemium). **Body** : `email`, `password`, `date_consentement_rgpd` (optionnel). **Mot de passe** : voir encadré RGPD ci-dessus.                                                                                                              |
...
| PATCH   | `/api/utilisateurs/me`                       | Tous               | Non                                             | Met à jour l'email et/ou le mot de passe du compte connecté. **Body** : `email`, `password` (optionnels). Si `password` est fourni, mêmes exigences RGPD que à l’inscription.                                                                                                                                                                       |
